    ...
    @if($event->id %2==0)
        <div class="card hoverable wow fadeInRight">
    @else
        ...
        @if(Auth::check())
            @if(Auth::user()->type == 'student')
                ...
                @if(!Auth::user()->hasRegisteredEvent($event->id) && !Auth::user()->hasConfirmed())
                    @if(!$event->isGroupEvent())
                        {{ link_to('#', 'ADD TO CART', ['class' => 'btn waves-effect waves-light green btn-register-event', 'data-event' => $event->id]) }}
                    @else
                        {{ link_to_route('pages.registerteam', 'ADD MY TEAM TO CART', ['event_id' => $event->id], ['class' => 'btn waves-effect waves-light green btn-registerteam-event', 'data-event' => $event->id]) }}
                    @endif
                @endif
            @endif
        @else
            {{ link_to_route('auth.login', 'LOGIN TO ADD', null,  ['class' => 'btn waves-effect waves-light red']) }}
        @endif
    </div>
</div>
